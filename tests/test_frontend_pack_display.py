...
def test_display_defaults_and_idempotent(tmp_path) -> None:
    ...
    assert display_payload["holder_name"] == "Unknown"
    ...
    assert manifest_entry["holder_name"] == "Unknown"
    ...
    assert manifest_display["holder_name"] == "Unknown"
    ...
    assert manifest_display["status"] == {
        "per_bureau": expected_per_bureau,
        "consensus": "--",
    }
...
def test_lean_pack_drops_consensus_defaults() -> None:
    ...
    assert display["account_number"] == {
        "per_bureau": per_bureau,
        "consensus": "--",
    }
    assert display["account_type"] == {
        "per_bureau": per_bureau,
        "consensus": "--",
    }
...
def test_example_account_16_pack_contains_per_bureau(tmp_path) -> None:
    ...
    if not sample_account_dir.exists():
        pytest.skip("sample account data not available")
    ...
    assert (
        display_payload["account_number"]["consensus"]
        == "440066**********"
    )
    assert (
